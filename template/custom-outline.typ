#let current-heading(level: auto, outlined: true) = {
  let here = here()
  query(heading.where(outlined: outlined).before(inclusive: false, here)).at(
    -1,
    default: none,
  )
}
  ...
  let headings = cx
    ...
    .map(hd => {
      ...
      let relation = if scope-path != none {
        ...
        let common-path = (
          hd.path.slice(0, path-len) == scope-path.slice(0, path-len)
        )
        ...
        let sibling = (
          not same
            and level == scope-path.len()
            and hd.path.slice(0, path-len - 1)
              == scope-path.slice(0, path-len - 1)
        )
